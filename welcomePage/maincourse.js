...
function getStatus(course){
    ...
    let paidCourse = paymentsData.find(payment => payment.courseName === course.title);
   console.log(paidCourse);
       if(currentuser.courses.find(studentCourse => studentCourse.ctitle === course.title)||paidCourse.paymentStatus==='Success'){
        return 'enrolled';
    }
    if(enrolledCourse){
        return 'pending';
